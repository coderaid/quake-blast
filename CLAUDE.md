...
- `src/weapon.ts` — `WeaponView`, the **fire feedback** system: pooled world-space hitscan tracers (an energy streak from just under the camera to the shot's endpoint) and impact sparks. There is deliberately **no visible gun model** — it's a kids' game. Purely visual — damage/cooldown stay in `Player`/`Game`; `Game.shoot()` calls `weapon.fireEffects(end, hitSomething)`.
