...
3. `won` — the wave is cleared with ≥1 base still standing (HUD reports how many of N bases survived).
4. `lost` — all bases destroyed, or the player died.
...
- `src/arena.ts` — `buildArena(scene)` builds the procedural **jungle level** and returns a contract the rest of the game wires into: `{ bound, update, perches, heightAt, pond }`. `bound` = clearing half-extent for movement/placement clamping; `update(dt)` animates water/wind/clouds every frame in every phase; `heightAt(x,z)` is the **terrain height field** (flat clearing = 0, rocky climbable hill rises) that the player, monsters, and brick placement all query; `pond` is a footprint the player and placement avoid; `perches` are tree-top points eagles roost on. Foliage is drawn with `InstancedMesh` (thousands of trees/understory at a handful of draw calls) and sways via a shared time uniform injected into the standard material shader (`onBeforeCompile`). There is still **no general collision system** — just AABB/plane clamping.
- `src/textures.ts` — shared **procedural canvas textures** (bark, leaf, rock, fur, feather), each built once and **cached in a module-level singleton** so thousands of instanced meshes reuse one texture. They are grayscale detail maps meant to multiply a mesh's base/instance color. Do **not** `dispose()` these per-entity — they're shared (see `Building.dispose`).
- `src/player.ts` — `Player`. The camera **is** the player (via `PointerLockControls`). Movement is velocity + damping on the camera rig, then clamped to `bound`; on top of that it does **jump + gravity over a support height** computed from `heightAt` and the tops of climbable bricks, stops at the pond bank, and resolves AABB collision against tall bricks. Its terrain/pond/brick dependencies are **injected** (`heightAt`, `pond`, and a `solids()` closure that returns live brick AABBs from `Game`). `tryShoot()` is a center-screen hitscan raycast; `aimGroundPoint()` intersects the center ray with y=0 for placement.
...
- Systems that need the world (terrain height, pond, brick AABBs, perches) get it **injected** from `Game` — they don't reach into arena internals. Keep new systems decoupled the same way.
